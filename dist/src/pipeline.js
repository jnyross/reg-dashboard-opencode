...
async function runPipeline(db, sourceIds) {
    ...
    for (let i = 0; i < targetSources.length; i += crawlConcurrency) {
        ...
        const batchResults = await Promise.allSettled(batch.map(async (source, batchIdx) => {
            const sourceIndex = i + batchIdx + 1;
            console.log(`[${sourceIndex}/${targetSources.length}] Crawling: ${source.name}...`);
            const crawlResult = await (0, crawler_1.crawlSource)(source);
        ...
        for (const batchResult of batchResults) {
            ...
            const sourceDbId = (0, db_1.upsertSource)(db, {
                sourceId: source.id,
                name: source.name,
                url: source.url,
    ...
    let queueIndex = 0;
    const workers = Array.from({ length: Math.min(analysisConcurrency, Math.max(1, totalItems)) }, async () => {
        while (true) {
            ...
            try {
                ...
                const analyzed = await (0, analyzer_1.analyzeItem)(item);
