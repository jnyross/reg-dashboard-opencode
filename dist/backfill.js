...
exports.backfillEventRiskAndJurisdiction = backfillEventRiskAndJurisdiction;
...
function buildSourceHostMap() {
    const map = new Map();
    for (const source of [...sources_1.sources, ...sources_1.twitterSearchSources]) {
...
function findSourceFallback(hostMap, sourceUrl) {
    if (!sourceUrl)
        return undefined;
...
function backfillEventRiskAndJurisdiction(db) {
    ...
    const tx = db.transaction((workRows) => {
        for (const row of workRows) {
            const fallbackFromHost = findSourceFallback(hostMap, row.source_url);
            const normalizedJurisdiction = (0, analyzer_1.resolveCanonicalJurisdiction)({
            ...
            const risk = (0, analyzer_1.deriveRiskScores)({
                text: [row.title, row.summary || "", row.business_impact || "", row.raw_content || ""].join("\n"),
                stage: row.stage,
            ...
            const jurisdictionChanged = row.jurisdiction_country !== normalizedJurisdiction.country ||
                (row.jurisdiction_state || null) !== (normalizedJurisdiction.state || null);
            const riskChanged = row.impact_score !== risk.impactScore ||
            ...
            update.run(normalizedJurisdiction.country, normalizedJurisdiction.state || null, risk.impactScore, risk.likelihoodScore, risk.confidenceScore, risk.chiliScore, now, row.id);
            updated += 1;
            if (jurisdictionChanged)
                jurisdictionUpdated += 1;
            if (riskChanged)
                riskUpdated += 1;
        }
    });
