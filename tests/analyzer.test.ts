...
function analyzeItem(item: CrawledItem) {
  ...
  const jurisdictionCountry =
    lowerContent.includes("european union") || lowerContent.includes("eu ")
      ? "European Union"
      : lowerContent.includes("united kingdom") || lowerContent.includes("uk ")
      ? "United Kingdom"
      : lowerContent.includes("australia")
      ? "Australia"
      : lowerContent.includes("canada")
      ? "Canada"
      : lowerContent.includes("united states") || lowerContent.includes("u.s.")
      ? "United States"
      : "Unknown";

  const ageBracket =
    lowerContent.includes("under 13") || lowerContent.includes("under 15") || lowerContent.includes("13-15")
      ? "13-15"
      : lowerContent.includes("under 16") || lowerContent.includes("under 18") || lowerContent.includes("16-18")
      ? "both"
      : lowerContent.includes("minor") || lowerContent.includes("child")
      ? "both"
      : "unknown";

  const stage =
    lowerContent.includes("passed") || lowerContent.includes("enacted") || lowerContent.includes("effective")
      ? "effective"
      : lowerContent.includes("committee")
      ? "committee_review"
      : lowerContent.includes("introduced") || lowerContent.includes("filed")
      ? "introduced"
      : lowerContent.includes("proposed") || lowerContent.includes("draft")
      ? "proposed"
      : "proposed";
...
function makeItem(overrides: Partial<CrawledItem>): CrawledItem {
  return {
    sourceId: "test",
...
describe("analyzer fallback logic", () => {
  test("should identify relevant item about Meta children regulation", () => {
    const item = makeItem({
      ...
      content:
        "The European Union has proposed new regulations requiring social media platforms like Meta to implement stricter age verification for children under 16.",
  ...
  test("should extract jurisdiction correctly", () => {
    const item = makeItem({
      ...
      content:
        "The UK government has announced new online safety legislation targeting social media platforms like Facebook and Instagram.",
      publishedDate: "2024-01-15",
    });
  ...
  test("should handle US jurisdiction", () => {
    const item = makeItem({
      ...
      content:
        "U.S. Congress members have introduced a bill to protect youth privacy online for teenagers and children on social media.",
      publishedDate: "2024-01-15",
    });
  ...
  test("should detect stage correctly", () => {
    const effectiveItem = makeItem({
      title: "Law becomes effective",
      content:
        "The regulation has been enacted and is now effective for all social media platforms serving children and teenagers.",
      publishedDate: "2024-01-15",
    });
