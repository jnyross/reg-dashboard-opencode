...
export function stripJavaScript(text: string): string {
  if (!text) return text;
  let cleaned = text;
...
export function stripHtmlAndCss(text: string): string {
  if (!text) return text;
  let cleaned = text;
...
export function isGarbageText(text: string): boolean {
  ...
  if (/\(function\s*\(/.test(text) || /googletagmanager/.test(text) || /a2a_config/.test(text)) return true;
  if (/otSDKStub/.test(text) || /sourceURL/.test(text) || /"@context"/.test(text)) return true;
...
export function generateSummaryFromContent(rawContent: string, title: string): string {
  if (!rawContent && !title) return "No summary available.";
  ...
  const sentences = cleaned.split(/[.!?]+/).filter(s => {
    ...
    if (/[{}]|function|var |const |let |import |export |module/.test(trimmed)) return false;
    if (/font-|margin|padding|border|color:|display:|width:|height:/.test(trimmed)) return false;
    if (/Skip to|Navigation|Menu|Cookie|Accept|Decline/.test(trimmed)) return false;
    if (/config|callback|overlay|template|querySelector|addEventListener/.test(trimmed)) return false;
    if (/sourceURL|sourceMap|webpack|imports/.test(trimmed)) return false;
    return true;
  });
  ...
  for (const sentence of sentences) {
    ...
    if (summary.length + trimmed.length > 400) break;
    summary += trimmed + ". ";
  }
...
export function isUnder16Related(title: string, summary: string, rawContent: string, sourceUrl: string): boolean {
...
export function runDataCleanup(db: DatabaseConstructor.Database): { cleaned: number; errors: string[] } {
  ...
  const transaction = db.transaction(() => {
    for (const row of rows) {
      try {
        const stillExists = getCurrentRowStmt.get(row.id) as { id: string } | undefined;
        if (!stillExists) {
          continue;
        }
        ...
        const shouldBeUnder16 = isUnder16Related(
          cleanedTitle,
          cleanedSummary || "",
          row.raw_content || "",
          row.source_url || ""
        ...
        try {
          ...
          if (duplicate) {
            const mergedSummary =
              !duplicate.summary || isGarbageText(duplicate.summary)
                ? cleanedSummary
                : cleanText(duplicate.summary);
            const mergedImpact =
              !duplicate.business_impact || isGarbageText(duplicate.business_impact)
                ? cleanedImpact
