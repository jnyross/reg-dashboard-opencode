...
const crawlState: { running: boolean; runId: number | null } = {
  running: false,
  runId: null,
};

function ensureAuxiliaryTables(db: DatabaseConstructor.Database): void {
  db.exec(`
  ...
  const existingConfig = db.prepare("SELECT id FROM alert_configs LIMIT 1").get() as { id: number } | undefined;
  if (!existingConfig) {
...
function parseStageList(value: string | undefined): Stage[] {
  return parseList(value).filter((entry): entry is Stage => allowedStages.includes(entry as Stage));
}

function parseAgeBracketList(value: string | undefined): string[] {
...
function sanitizeEvent(row: DbEventRow) {
  ...
  const normalizedAgeBracket =
    isUnder16Applicable && row.age_bracket === "unknown" ? "both" : row.age_bracket;
...
function finishCrawlRun(
  ...
  db.prepare(
    ...
    payload.status,
    completedAt,
    payload.sourcesProcessed ?? null,
    payload.itemsCrawled ?? null,
    payload.itemsAnalyzed ?? null,
    payload.itemsSaved ?? null,
    payload.errorCount ?? null,
...
function getLatestCrawlRun(db: DatabaseConstructor.Database): CrawlRunRow | undefined {
  return db.prepare("SELECT * FROM crawl_runs ORDER BY id DESC LIMIT 1").get() as CrawlRunRow | undefined;
}

function buildWhereClause(filters: EventFilters): { where: string; params: (string | number)[] } {
  ...
  if (filters.search) {
    ...
    clauses.push(
      "(lower(e.title) LIKE ? OR lower(COALESCE(e.summary, '')) LIKE ? OR lower(COALESCE(e.business_impact, '')) LIKE ? OR lower(COALESCE(e.raw_content, '')) LIKE ? OR lower(COALESCE(e.source_url, '')) LIKE ?)"
    );
...
function setPaginationHeaders(res: Response, page: number, limit: number, total: number): void {
  ...
  const links: string[] = [];
  if (page > 1) links.push(`</api/events?page=${page - 1}&limit=${limit}>; rel=\"prev\"`);
  if (page < totalPages) links.push(`</api/events?page=${page + 1}&limit=${limit}>; rel=\"next\"`);
  if (links.length > 0) {
...
function buildSimplePdf(title: string, contentLines: string[]): Buffer {
  const lines = [title, "", ...contentLines].flatMap((line) => wrapText(line, 92)).slice(0, 58);

  const streamLines: string[] = ["BT", "/F1 11 Tf", "50 780 Td", "14 TL"];
...
async function sendWebhookIfConfigured(
  ...
  const config = db.prepare("SELECT webhook_url FROM alert_configs LIMIT 1").get() as {
    webhook_url: string | null;
  };
...
function createHighRiskNotifications(
  db: DatabaseConstructor.Database,
  startedAt: string,
  minChili: number
): number {
  const highRiskRows = db
    ...
    .all(startedAt, minChili) as Array<{ id: string; title: string; chili_score: number }>;

  const now = new Date().toISOString();
  const insert = db.prepare(
    "INSERT INTO notifications (title, message, severity, event_id, created_at) VALUES (?, ?, ?, ?, ?)"
  );

  for (const row of highRiskRows) {
    const severity: "warning" | "critical" = row.chili_score >= 5 ? "critical" : "warning";
    insert.run(
      "High-risk regulation detected",
      `${cleanTitle(row.title)} (${row.chili_score}🌶️)`,
      severity,
      row.id,
      now
    );
...
export function createApp(db: DatabaseConstructor.Database) {
  ...
  app.get("/api/events", (req: Request, res: Response) => {
    ...
    const total = (
      db.prepare(`SELECT COUNT(*) AS total FROM regulation_events e ${where}`).get(...params) as { total: number }
    ).total;
  ...
  app.get("/api/events/:id", (req: Request, res: Response) => {
    const row = db
      ...
      .get(req.params.id) as DbEventRow | undefined;
    ...
    const feedbackRows = db
      ...
      .all(req.params.id) as FeedbackRow[];
    ...
    const relatedRows = db
      ...
      .all(req.params.id, row.jurisdiction_country, row.stage, `%${cleanTitle(row.title).slice(0, 16).toLowerCase()}%`) as DbEventRow[];
    ...
    const timelineRows = db
      ...
      .all(req.params.id) as TimelineRow[];
  ...
  app.put("/api/events/:id", (req: Request, res: Response) => {
    ...
    const current = db
      ...
      .get(id) as { id: string; stage: Stage } | undefined;
    ...
    `
      ...
      payload.title ? cleanTitle(payload.title) : null,
      payload.summary ? cleanText(payload.summary) : null,
      payload.businessImpact ? cleanText(payload.businessImpact) : null,
      payload.stage ?? null,
      payload.ageBracket ?? null,
      payload.isUnder16Applicable === undefined ? null : payload.isUnder16Applicable ? 1 : 0,
      requiredSolutions ?? null,
    ...
    if (payload.stage && payload.stage !== current.stage) {
      ...
      ).run(id, current.stage, payload.stage, now, "Analyst edit");
    }
  ...
  app.post("/api/crawl", async (req: Request, res: Response) => {
    ...
    const sourceIds = Array.isArray(body.sourceIds)
      ? body.sourceIds.filter((entry): entry is string => typeof entry === "string" && entry.length > 0)
      : undefined;
    ...
    try {
      ...
      const status: "completed" | "partial" = result.errors.length > 0 ? "partial" : "completed";
  ...
  app.get("/api/analytics/summary", (req: Request, res: Response) => {
    const row = db
      ...
      .get() as {
        ...
        newestEventUpdatedAt: string | null;
      };
    ...
    const topJurisdiction = db
      ...
      .get() as { country: string; count: number } | undefined;
  ...
  app.get("/api/analytics/trends", (req: Request, res: Response) => {
    const trends = db
      ...
      .all() as Array<{ month: string; count: number; avgRisk: number; highRiskCount: number }>;
  ...
  app.get("/api/analytics/jurisdictions", (req: Request, res: Response) => {
    const jurisdictions = db
      ...
      .all() as Array<{ country: string; count: number; avgRisk: number; maxRisk: number; highRiskCount: number }>;
  ...
  app.get("/api/analytics/stages", (req: Request, res: Response) => {
    const stages = db
      ...
      .all() as Array<{ stage: Stage; count: number; avgRisk: number }>;
  ...
  app.post("/api/alerts/config", (req: Request, res: Response) => {
    ...
    const now = new Date().toISOString();
    const current = db.prepare("SELECT * FROM alert_configs LIMIT 1").get() as AlertConfigRow;
    ...
    `
      ...
      body.minChili ?? current.min_chili,
      body.webhookUrl === undefined ? current.webhook_url : body.webhookUrl,
      now,
      current.id
    );

    const updated = db.prepare("SELECT * FROM alert_configs WHERE id = ?").get(current.id) as AlertConfigRow;
    res.json(updated);
  });
  ...
  app.get("/api/email-digest/preview", (req: Request, res: Response) => {
    ...
    const lines = [
      ...
      ...events.slice(0, 10).map(
        (event, index) =>
          `${index + 1}. ${cleanTitle(event.title)} (${event.chili_score}🌶️, ${event.jurisdiction_country}, ${event.stage})`
      ),
    ];
  ...
  app.get("/api/reports/trend-analysis", (req: Request, res: Response) => {
    const trends = db
      .prepare(
        `
        SELECT
  ...
  app.get("/api/reports/custom", (req: Request, res: Response) => {
    ...
    if (format === "csv") {
      ...
      for (const row of rows) {
        lines.push(finalFields.map((field) => escapeCell((row[field] ?? "") as string | number | null)).join(","));
      }
