...
const ageBracketValues: AgeBracket[] = ["13-15", "16-18", "both", "unknown"];
const statusValues = ["new", "updated", "status_changed", "unchanged"] as const;

type LawStatus = (typeof statusValues)[number];
...
export interface LawSyncResult {
  scanned: number;
  insertedLaws: number;
  insertedUpdates: number;
  linkedEvents: number;
...
export function extractCanonicalLawTitle(title: string): string {
  const cleanedTitle = cleanTitle(title || "");
  if (!cleanedTitle) return "Untitled Law";
  ...
  if (!normalized) return cleanedTitle;
...
export function buildLawKey(country: string, state: string | undefined, canonicalTitle: string): string {
  const keySeed = [normalizeKeyPart(country), normalizeKeyPart(state), normalizeKeyPart(canonicalTitle)].join("|");
  const digest = crypto.createHash("sha1").update(keySeed).digest("hex").slice(0, 20);
  return `${normalizeKeyPart(country) || "unknown"}|${normalizeKeyPart(state) || ""}|${digest}`;
}

function buildLawUpdateKey(lawKey: string, law: LawInput, canonicalTitle: string): string {
  const seed = [
    lawKey,
...
function hasColumn(db: DatabaseConstructor.Database, table: string, column: string): boolean {
  const rows = db.prepare(`PRAGMA table_info(${table})`).all() as Array<{ name: string }>;
  return rows.some((row) => row.name === column);
}

export function ensureLawSchema(db: DatabaseConstructor.Database): void {
...
function normalizeLawInput(input: LawInput): LawInput {
  const cleanedTitle = cleanTitle(input.title);
  const cleanedSummary = cleanText(input.summary || "");
  const safeSummary = cleanedSummary && !isGarbageText(cleanedSummary)
    ? cleanedSummary
    : generateSummaryFromContent(input.rawContent || "", cleanedTitle);

  const cleanedImpact = cleanText(input.businessImpact || "");
  const safeImpact = cleanedImpact && !isGarbageText(cleanedImpact)
    ? cleanedImpact
    : "Requires review for compliance impact on Meta platforms.";

  const cleanedRaw = cleanText(input.rawContent || "").slice(0, 10000);
  const inferredUnder16 = isUnder16Related(cleanedTitle, safeSummary, cleanedRaw, input.sourceUrl);
...
function refreshLawSnapshotFromLatestUpdate(db: DatabaseConstructor.Database, lawId: number): void {
  ...
  const counts = db.prepare("SELECT COUNT(*) AS count FROM law_updates WHERE law_id = ?").get(lawId) as { count: number };
  ...
  db.prepare(
    ...
    latestUpdate.title,
    latestUpdate.canonical_title,
    latestUpdate.stage,
    latestUpdate.age_bracket,
    latestUpdate.is_under16_applicable,
    latestUpdate.impact_score,
    latestUpdate.likelihood_score,
    latestUpdate.confidence_score,
    latestUpdate.chili_score,
    latestUpdate.summary,
    latestUpdate.business_impact,
    latestUpdate.required_solutions,
    latestUpdate.competitor_responses,
    latestUpdate.effective_date,
    latestUpdate.published_date,
    latestUpdate.source_url,
    latestUpdate.reliability_tier,
    latestUpdate.status,
    latestUpdate.published_date || latestUpdate.captured_at,
    counts.count,
    now,
    lawId,
  );
...
export function upsertLawWithUpdate(
  db: DatabaseConstructor.Database,
  input: LawInput,
  eventId?: string,
): { lawId: number; insertedLaw: boolean; insertedUpdate: boolean; lawKey: string } {
  ...
  let lawRow = db.prepare(
    `SELECT id, first_seen_at FROM laws WHERE law_key = ? LIMIT 1`,
  ).get(lawKey) as { id: number; first_seen_at: string } | undefined;
  ...
  if (!lawRow) {
    ...
    const result = insertLaw.run(
      ...
      canonicalTitle,
      law.title,
      law.jurisdictionCountry,
      law.jurisdictionState || null,
      law.stage,
      law.ageBracket,
      law.isUnder16Applicable ? 1 : 0,
      law.impactScore,
      law.likelihoodScore,
      law.confidenceScore,
      law.chiliScore,
      law.summary,
      law.businessImpact,
      JSON.stringify(law.requiredSolutions),
      JSON.stringify(law.competitorResponses),
      law.effectiveDate || null,
      law.publishedDate || null,
      law.sourceUrl,
      law.reliabilityTier,
      law.status,
      law.lastCrawledAt || null,
      createdAt,
      latestUpdateAt,
      createdAt,
      law.updatedAt,
  ...
  const existingUpdate = db.prepare(
    `SELECT id FROM law_updates WHERE update_key = ? LIMIT 1`,
  ).get(updateKey) as { id: number } | undefined;
...
function mergeDuplicateLaws(db: DatabaseConstructor.Database): number {
  ...
  for (const keyRow of duplicateKeys) {
    const candidates = db.prepare(
      ...
      `,
    ).all(keyRow.law_key) as Array<{ id: number; latest_update_at: string; update_count: number }>;
...
export function syncLawsFromEvents(db: DatabaseConstructor.Database): LawSyncResult {
  ensureLawSchema(db);
  ...
  const tx = db.transaction(() => {
    for (const row of rows) {
      ...
      if (result.insertedLaw) insertedLaws += 1;
      if (result.insertedUpdate) insertedUpdates += 1;
      linkedEvents += 1;
    }
  });
