...
export function initializeSchema(db: DatabaseConstructor.Database): void {
  ...
  const existingAlertConfig = db.prepare("SELECT id FROM alert_configs LIMIT 1").get() as { id: number } | undefined;
...
export function upsertSource(
  ...
  if (existing) {
    ...
    `).run(
      ...
      source.jurisdictionCountry,
      source.jurisdictionState || null,
...
function normalizeSummary(item: AnalyzedItem): string {
  const candidate = cleanText(item.summary || "");
  if (!candidate || isGarbageText(candidate)) {
    return generateSummaryFromContent(item.rawContent, item.title);
  }
  return candidate;
}

function normalizeBusinessImpact(item: AnalyzedItem): string {
  const candidate = cleanText(item.businessImpact || "");
  if (!candidate || isGarbageText(candidate)) {
...
export function upsertAnalyzedItem(
  ...
  const existing = db.prepare(
    `SELECT id, stage, created_at
     ...
     LIMIT 1`
  ).get(item.jurisdictionCountry, jurisdictionState, cleanedTitle) as {
    id: string;
    stage: string;
    created_at: string;
  } | undefined;
  ...
  const inferredUnder16 = isUnder16Related(cleanedTitle, cleanedSummary, cleanedRawContent, item.url);
  ...
  if (isNew) {
    ...
    ).run(eventId, null, item.stage, now, "Initial ingestion");
